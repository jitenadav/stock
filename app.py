...
def adminDashboard():
    if 'username' in session:
        ...
        # images = mongo.db.user_images
        # to_validate = images.find({'isValidated':0})
        #
        # for item in to_validate:
        #     ID = item['_id']
        #     extractor.analyze(item['imgDescription'], candidate_pos = ['NOUN', 'PROPN','VERB'], window_size=4, lower=False)
        #     highlights = extractor.get_keywords(12)
        #     images.update_one({'_id': ObjectId(ID)},{'$set':{'key_highlight':highlights, 'isValidated':1}})
...
def to_db():
    if 'userImage' in request.files:
        ...
        if userImage and allowed_file(userImage.filename):
            ...
            userImage.save(os.path.join(app.config['UPLOAD_FOLDER'], imgName))
            caption_dict = run(path+imgName)
            mongo.save_file(imgName, request.files['userImage'])
        ...
        mongo.db.user_images.insert_one({'uname':session['username'], 'userImage_name': imgName, 'imgDescription' : imgDesc ,'imgCategory' : imgCat,
                                            'isValidated':0,'isVerified':0,'key_highlight':highlights,'captions':captions})
...
def user_resubmit():
    ...
    #         mongo.db.user_images.update_one({"_id" :id}, {"$set": {'userImage_name': imgName,'imgDescription' : imgDesc ,'imgCategory' : imgCat}}, upsert=True)
    #
...
#
#     # test = "Hello There"
#     return render_template('display.html', test = iDlist, len = len(iDlist),imgDesc = imgDesc, imgCategory = imgCategory)
#     # return jsonify({'Image Names : ':iDlist, 'Image description':imgDesc, 'Image Category':imgCategory})
#
#
# # ----------------------------------------------------------------------------------------#
